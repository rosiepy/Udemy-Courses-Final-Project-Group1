...
def create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('index')  # Redirect to a page where you list all courses after creating one
    else:
...
def delete(request, course_id):
    ...
    course = get_object_or_404(Udemy, pk=course_id)
    if request.method == 'POST':
        #course.price.delete()
        #course.level.delete()
        course.delete()
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            if user.is_active:
                ...
                return redirect('courses:register')  # Replace 'reg
